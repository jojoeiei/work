...
public class ReregistrationPageController {
    @FXML
    ...
    @FXML
    private ImageView uploadProflieImageView; //ภาพคนใช้งาน
    @FXML
    ...
    @FXML
    protected void onUploadProfileBtnPic(){ // Upload Profile Pic Btn
        try{
            FXRouter.goTo(""); // todo
        } catch(IOException e){
        ...
        if(usernameTextRegister != null && passwordTextRegister != null && confirmPasswordTextRegister != null){ // Check if Text Field is not blank
            ...
            if (!passwordStatus && !usernameStatus) {
               errorLabel.setText("Username Is Taken And Password Are Not Match");
                ...
                userList.addNewUser(usernameTextRegister, nameTextRegister, passwordTextRegister, loginTimeTextRegister);
